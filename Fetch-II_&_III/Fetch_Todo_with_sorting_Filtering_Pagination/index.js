...
const TodoAssignee = document.getElementById('assignee');
...
function DisplayTodo(todos) {
    ...
    todos.forEach((todo, idx) => {
        ...
        Assignee.textContent = `Assignee : ${todo.assignee}`;
        ...
        TodoDiv.append(Title, Assignee, Status, updateBtn, deleteBtn);
        AllTodos.append(TodoDiv);
    });
}

async function filteredDatas(current_page, todos_per_page) {
    try{
        const response = await fetch(`http://localhost:3000/todos?_page=${current_page}&_per_page=${todos_per_page}`);
        const data = await response.json();
        console.log(data);
        
        const filteredData=data.data.filter(el => el.status==false)
...
filteringBtn.addEventListener('click',()=>{
    filteredDatas(current_page, todos_per_page);
})

let sortOrder='asc';

async function sortingDatas(current_page, todos_per_page) {

   const sortParameters=sortOrder == 'asc' ? 'title' : '-title';
   sortOrder=(sortOrder==='asc') ? 'desc' : 'asc';

    try{
        const response = await fetch(`http://localhost:3000/todos?_page=${current_page}&_per_page=${todos_per_page}&_sort=${sortParameters}`);
        const data = await response.json();
        // console.log(data);
...
sortingBtn.addEventListener('click',()=>{
    sortingDatas(current_page, todos_per_page);
})


async function deleteTodo(todos_id) {
    try {
        const response = await fetch(`http://localhost:3000/todos/${todos_id}/`, {
            method: "DELETE",
            headers: {
        ...
        const data = await response.json();
        FetchTodo(current_page, todos_per_page);
...
async function AddTodo() {
    ...
    const TodoAssigneeValue = TodoAssignee.value;
...
async function FetchTodo(current_page, todos_per_page) {
    try {
        const response = await fetch(`http://localhost:3000/todos?_page=${current_page}&_per_page=${todos_per_page}`);
        const data = await response.json();
        DisplayTodo(data.data);
